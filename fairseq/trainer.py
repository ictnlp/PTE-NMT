...
class Trainer(object):
    ...
    def __init__(self, args, task, model, model_teacher, criterion, dummy_batch=None, oom_batch=None):
        self.args = args
        self.task = task
    ...
    def load_checkpoint(
        ...
        if os.path.exists(filename):
            ...
            try:
                if not model_teacher:
                    self.get_model().load_state_dict(state['model'], strict=True)
                else:
                    self.model_teacher.load_state_dict(state['model'], strict=True)
            except Exception:
                raise Exception(
        ...
        for i, sample in enumerate(samples):
            ...
            try:
                ...
                loss, sample_size, logging_output = self.task.train_step(
                    sample, self.model, self.model_teacher, self.criterion, self.optimizer,
                    ignore_grad
                )
